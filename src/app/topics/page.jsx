    ...
    return(
        <>
            ...
            <Main className=" w-[100%] h-[auto]   bg-gradient-to-r from-[#6b5ec3] to-[#9788f8] min-h-screen;">
                <TitleOne className="text-[#FFFFFF] text-2xl text-center mb-9 topics-title ">Here you'll find the best ESL conversation questions — click a topic to dive in!</TitleOne>
                <Section 
                ...
                className=" w-[70%] mx-auto grid grid-cols-4 place-items-center shadow-xl shadow-[#9788f8] rounded-2xl pt-4 topics-list"  >
                    {topics.map((topic) => (
                        <Link className="text-[#FFFFFF] hover:scale-105 transition-transform duration-500 font-Raleway p-2" href={`/topics/${topic.slug}`} key={topic.slug} >{topic.name}</Link>
                    ))}
